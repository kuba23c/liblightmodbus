...
static bool notify_wait(void) {
	modbus_rtu.events = osThreadFlagsWait(MODBUS_RTU_READ_READY | MODBUS_RTU_EMIT_READY, osFlagsWaitAny, osWaitForever);
...
static void wait_while_uart_busy(void) {
	...
	do {
		if (modbus_rtu.uart.uart->Instance->SR & UART_FLAG_TC) {
			break;
		}
...
static void send_byte(void) {
	modbus_rtu.send_cnt++;
	modbus_rtu.uart.uart->Instance->DR = (uint32_t) (modbus_rtu.send_buffer[modbus_rtu.send_cnt - 1]);
}
...
static void modbus_rtu_uart_irq_set(modbus_irq_t irq_state) {
	...
	case MODBUS_IRQ_ALL_OFF:
		...
		dummy_read = (uint8_t) (modbus_rtu.uart.uart->Instance->DR);
...
void USART3_IRQHandler(void) {
	uart_isr_status = modbus_rtu.uart.uart->Instance->SR;
	...
	if (uart_isr_status & UART_FLAG_RXNE) {
		if (modbus_rtu.receive_buffer_len < sizeof(modbus_rtu.receive_buffer)) {
			...
			case MODBUS_RTU_IDLE:
				modbus_rtu.receive_buffer[modbus_rtu.receive_buffer_len] = (uint8_t) (modbus_rtu.uart.uart->Instance->DR);
